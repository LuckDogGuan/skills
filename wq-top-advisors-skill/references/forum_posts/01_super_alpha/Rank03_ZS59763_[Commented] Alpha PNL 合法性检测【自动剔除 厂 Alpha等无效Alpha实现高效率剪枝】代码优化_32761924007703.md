...
- **链接**: https://support.worldquantbrain.com[Commented] Alpha PNL 合法性检测【自动剔除 厂 Alpha等无效Alpha实现高效率剪枝】代码优化_32761924007703.md
...
### 评论 #16 (作者: 顾问 JR23144 (贺六浑) (Rank 6), 时间: 10个月前)
...
> group_twe.ipynb
> test py 1
> diagnose_alpha_axxazww py
> Iodedaima
> group_twe.ipynb
> 得根据这个进行
> 个大致的修改。要先过自相关检测再聚合 (未修改)
> 代码
> 十 Markdown
> D 全部运行
> S重启
...
> 2025
> 08-31 22:03:00
> 2025-08-31  22:03:00  1756648980
> [ 'EnlwAgO'
> alpha
> group_neutralize(ts_delta(anl4_fcfps_flag 20), industry);alpha_Bpm
> group_mean(alpha,
> 1, industry)
> [ 'QROrw65
> alpha
> Broup_scale(ts_zscore(anl4_tbve_ft, 20), industry);alpha_gpm
> group_mean(alpha,
> 1, industry);resid
> [ 'w7Lgngy
> alpha
...
> industry)
> alpha_gpm
> group_mean(alpha, 1, industry);resid
> [ '13XIgVR
> alpha
> group_scale(ts_scale(anl4_bvps_flag, 252), industry);alpha_gpm
> group_mean(alpha, 1, industry);resid
> [ 'Y35V5oq
> alpha
> group_zscore(ts_scale(anl4_bvps_flag〉 120), industry);alpha_Bpm
> group_mean(alpha, 1, industry);resi
> [ '3mgryzg
> alpha
> group_scale(ts_scale(anl4_bvps_flag, 120), industry);alpha_gpm
> group_mean(alpha, 1, industry);resid
> [ 'eMOqZMJ
> alpha
> group_scale(ts_scale(anl4_bvps_flag; 252), industry);alpha_Bpm
> group_mean(alpha, 1, industry);resid
> [ 'ojkzQlk
> alpha
> group_zscore(ts_scale(anl4_bvps_
> 120), industry ) ;alpha_Bpm
> group_mean(alpha,
> industry);resi
> [ 'pbGxdgv
> alpha
> group_scale(ts_scale(anl4_bvps
> 120),
> industry) ;alpha_gpm
> group_mean(alpha, 1, industry)
> resid
> [ 'X7IWZRq
> alpha
> group_zscore(ts_scale(anl4_cfo
> 120),
> industry);alpha_Bpm
> group_mean(alpha,
> 1, industry);resid
> [ 'voyzeb
> alpha
> group_zscore(ts_scale(anl4_cff
> 120),
> industry);alpha_Bpm
> group_mean(alpha,
> industry)
...
> [ 'MvOLkko
> alpha
> group_scale(ts_scale(anl4_cff
> 120)
> industry );alpha_gpm
> group_mean(alpha, 1, industry);resid
> [ 'AXQ7OGY
> alpha
> group_scale(ts_scale(anl4_cfo_flag, 120), industry);alpha_Bpm
> group_mean(alpha,
> 1, industry);resid
> ['JaQ5PjE
> alpha
> group
> Zscore(ts_scale(anl4_Cfo_flag; 120), industry);alpha_Bpm
> group
> mean(alpha, 1, industry);resid
> [ 'AXvaj8Q
> alpha
> group_scale(ts_scale(anl4_cfo_
> 120)
> industry );alpha_gpm
> group_mean(alpha, 1, industry);resid
> [ 'WTYOVZP
...
> [ 'PLJryL
> alpha
> group_zscore(ts_scale(anl4_cfo_
> 120), industry) ;alpha_BPm
> group_mean(alpha, 1, industry);resid
> 281  个不合法的pn1,己被剔除
> 获取到  169 个ID:
